resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name = "web-${var.env}.${var.zone_name}" #web-dev.surisetty.online
    origin_id   = "web-${var.env}.${var.zone_name}"
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"] #web-cdn.surisetty.online

  default_cache_behavior {
    allowed_methods  = ["DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "POST", "PUT"]
    cached_methods   = ["GET", "HEAD"]
    target_origin_id = "web-${var.env}.${var.zone_name}"
...
module "records" {
  ...
  records = [
    {
      name            = "web-${var.common_tags.Component}"#web-cdn.surisetty.online
      type            = "A"
